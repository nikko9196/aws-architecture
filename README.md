# AWS Architecture Projects

A collection of AWS architecture and serverless projects completed during the **AWS re/Start programme** at **Unitec (October 2023 – February 2024).**

This repository demonstrates:

- Cloud Architecture Design
- AWS Networking and VPC Design
- Multi-AZ Infrastructure Planning
- Serverless Application Development
- Event-Driven Architecture
- Infrastructure as Code
- AWS Security and Networking Concepts
- Scalable and Cost-Optimised AWS Solutions

## Programming and Tools

- Python
- AWS Services
- Serverless Framework

## Project Highlights

### Part I – Classic AWS Architecture Design

Design of a scalable multi-tier AWS infrastructure featuring VPC design, subnet planning, security groups, route tables, high availability, and network traffic flow scenarios.

📄 [View Part I](./Part%20I)

### Part II – AWS Native Architecture Design

Design of a fully managed, serverless AWS architecture for a large-scale news aggregation platform using AWS native services.

📄 [View Part II](./Part%20II)

### Part III – Serverless Event-Driven Image Processing Pipeline

Implementation of an event-driven image processing workflow using Amazon S3, AWS Lambda, and Amazon EventBridge with success and failure event handling.

📄 [View Part III](./Part%20III)

## Repository Structure

```text
AWS-ARCHITECTURE/
│
├── Part I/
│   Classic AWS Architecture Design
│
├── Part II/
│   AWS Native Architecture Design
│
└── Part III/
    Serverless Event-Driven Image Processing Pipeline
```

## Notes

These projects were originally completed as coursework during the AWS re/Start programme and have been reorganised and documented for portfolio presentation purposes.
